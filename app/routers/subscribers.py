...
# For testing purposes: Endpoint for Returning all emails in database
...
# Protected Endpoint for Deleting an Email from the database
@router.delete("/unsubscribe", response_model = schemas.SubscriberOut)
def unsubscribe(current_user_email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    db_subscriber = crud.get_subscriber_by_email(current_user_email, db)
